...
$(function() {
    $('.submitBtn').each(function() {
      $(this).click(function() {
        //var keyValue = ($(this).parent().prev().prev().children().html() + ' to do');//why localStorage.setItem doesn't take this value? 
        var keyName = ($(this).parent().prev().children().val()); 

        localStorage.setItem(($(this).parent().prev().prev().children().html() + ' to do'),keyName)
        renderLastSavedValue()
      }); 
    });
  });
  ...
  $(function renderLastSavedValue(){
    $('.task').each(function(){
        var savedKeyName = ($(this).parent().prev().children().html() + 'to do')
        ($(this).html()) = localStorage.getItem(savedKeyName)
    })
  });
